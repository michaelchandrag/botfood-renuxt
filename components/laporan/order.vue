  ...
  <div
    ...
    <form class="w-full max-w-m">
      <div class="md:flex md:items-center mb-6">
        <div class="md:w-full">
          <label class="block font-bold mb-1 md:mb-0 pr-4"> Pesanan </label>
          <label>
            Daftar pesanan dalam jangka waktu yang telah ditentukan.
          </label>
        </div>
      </div>
      <div class="md:flex md:items-center mb-6">
        ...
        <div class="md:w-2/12">
          <client-only>
            <date-picker v-model="filters.fromDate" input-class="border-date" />
          </client-only>
        </div>
        <div class="md:w-2/12">
          <label
        ...
        <div class="md:w-3/12">
          <div class="">
            <button
              v-if="isDownload"
              class="cursor-not-allowed shadow bg-gray-500 hover:bg-gray-400 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded"
              @click="">
...
export default {
  ...
  methods: {
    download() {
      ...
      var queryParams = {
        from_ordered_at: this.$moment(this.filters.fromDate).format(
          'YYYY-MM-DD 00:00:00'
        ),
        until_ordered_at: this.$moment(this.filters.untilDate).format(
          'YYYY-MM-DD 23:59:59'
        ),
        xlsx: true,
      }
      ...
      this.$axios({
        method: 'GET',
        url: `me/report/orders?${queryParams}`,
        responseType: 'blob',
      }).then((r) => {
        ...
        fileLink.href = url
        fileLink.setAttribute('download', 'rekap-pesanan.xlsx')
