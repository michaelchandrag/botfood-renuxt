    ...
    <div class="flex flex-col w-full mb-4">
      ...
      <div v-if="!loading" class="bg-white rounded-md p-4">
        <span class="items-center text-orange-500 inline-block align-middle">
          ...
          <span class="ml-2 text-green-500">Trend Rating Terbaru</span>
        </span>
        <div class="h-6"></div>
        <div class="grid grid-cols-1 bg-white rounded-md md:grid-cols-3 w-full gap-4">
          <div class="rounded-fd bg-white text-sm border w-full p-2">
            ...
            <ul class="mt-3">
              <li v-for="trend in gofood_trends" class="flex justify-between text-s border-b py-2">
                ...
                <div class="flex gap-x-1 text-xs">
                  <span>{{dateToHuman(trend.created_at)}}</span>
                </div>
              </li>
            </ul>
          </div>
          <div class="rounded-fd bg-white text-sm border w-full p-2">
            ...
            <ul class="mt-3">
              <li v-for="trend in grabfood_trends" class="flex justify-between px-2  text-s border-b py-2 relative">
                <div class="flex gap-x-1 items-center">
                  <div>
          ...
          <div class="rounded-fd bg-white text-sm border w-full p-2">
            ...
            <ul class="mt-3">
              <li v-for="trend in shopeefood_trends" class="flex justify-between px-2  text-s border-b py-2 relative">
                <div class="flex gap-x-1 items-center">
                  <div>
...
export default {
  ...
  methods: {
    dateToHuman (datetime) {
      return this.$moment(datetime).fromNow()
    },
    async getData() {
      try {
        this.loading = true;
        const res = await this.$axios.get("/me/report/rating_latest_trends");
        this.loading = false;

        if (res.data.success) {
          this.data = res.data.data;
          this.gofood_trends = this.data.gofood_trends
          this.grabfood_trends = this.data.grabfood_trends
          this.shopeefood_trends = this.data.shopeefood_trends
        } else {
