    ...
    <div class="w-10/12 bg-gray-200 pl-6" style="padding-top: 40px">
      ...
      <div class="flex flex-col">
        <div class="grid grid-cols-2 gap-4 mr-6">
          ...
          <div
            v-if="isLoading"
            class="bg-gray-300 h-48 rounded-fd animate-pulse"></div>
          <div
            v-if="isLoading"
            class="bg-gray-300 h-48 rounded-fd animate-pulse"></div>
        </div>
        ...
        <div
          v-if="isLoading"
          class="bg-gray-300 h-64 rounded-fd animate-pulse mr-6"></div>
        <!-- table start -->
              ...
              <div
                ...
                <ul class="w-full border-gray-300">
                  <li
                    class="bg-white px-4 py-3 w-full"
                    @click.prevent="
                      ;(statusDropdown = false), (isOutletOpen = 1), getData()
                    ">
                    Buka
                  </li>
                  <li
                    class="bg-white px-4 py-3 w-full"
                    @click.prevent="
                      ;(statusDropdown = false), (isOutletOpen = 0), getData()
                    ">
                    Tutup
                  </li>
                  <li
            ...
            <table class="table-auto w-full">
              <thead class="border-b">
                <tr>
                  ...
                  <th
                    class="py-4 text-text cursor-pointer"
                    :class="sortKey === 'items_percentage' ? 'filter' : ''"
                    @click.prevent="
                      ;(sortKey = 'items_percentage'),
                        sortValue === 'desc'
                          ? (sortValue = 'asc')
                          : (sortValue = 'desc'),
                        getData(true)
              ...
              <tbody v-if="listLoading">
                <tr v-for="n in 10" :key="n" class="h-12">
              ...
              <tbody v-if="!listLoading">
                <tr
                  v-for="(channel, index) in data.branch_channels"
                  :key="channel.id"
                  ...
                  <td class="text-center text-text p-4">
                    {{ channel.channel }}
                    <span
                      v-if="channel.gobiz_is_connected == 1"
                  ...
                  <td class="text-center text-text p-4">
                    <span v-if="channel.is_open" class="text-green-500"
                      >Buka</span
                    >
                  ...
                  <td class="text-center text-text p-4">
                    <span
                      :class="{
                        'text-green-food':
                          parseInt(channel.items_percentage) >= 90,
                        'text-yellow-600':
                          parseInt(channel.items_percentage) >= 50 &&
                          parseInt(channel.items_percentage) < 90,
                        'text-red-600': parseInt(channel.items_percentage) < 50,
                      }">
                      {{ `${channel.items_active}/${channel.items_total}` }} ({{
                  ...
                  <td class="text-center text-text p-4">
                    <button
                      class="ml-4 focus:outline-none mr-2 bg-yellow-200 text-black-500 text-xs rounded-full px-2 py-1"
                      @click.prevent="showItem(channel.id, index)">
                      <i class="fas fa-eye"></i> Menu
                    </button>
                    <button
                      v-if="
                        channel.channel == 'GoFood' &&
                        channel.gobiz_is_connected == 0
                      "
                      class="ml-4 focus:outline-none mr-2 bg-blue-200 text-black-500 text-xs rounded-full px-2 py-1"
                      @click.prevent="checkSeamless(channel)">
                      <i class="fas fa-upload"></i> Hubungkan
                    </button>
                    <button
                      v-if="channel.gobiz_is_connected == 1"
                      class="ml-4 focus:outline-none mr-2 text-black-500 bg-green-300 text-xs rounded-full px-2 py-1"
                      @click.prevent="handleOpenStatus(channel, false)">
                      <i class="fas fa-store"></i> Ubah Status
                    </button>
                    <nuxt-link
                      :to="'/outlet/' + channel.id"
                      class="bg-gray-200 text-black-500 text-xs rounded-full px-2 py-1"
                      >Detail <i class="fas fa-ellipsis-h"></i
                    ></nuxt-link>
                  </td>
                </tr>
              </tbody>
      ...
      <div
        v-if="isShowItems"
        class="fixed top-0 flex items-center z-40 left-0 w-screen h-screen">
        <div class="w-1/2 bg-white rounded-fds z-40 mx-auto">
          <div class="p-4">
            <div class="bg p-6 relative rounded-fd flex gap-10 items-center">
              <div class="absolute top-0 right-0">
                <button
                  class="text-white text-sm px-3 py-1 text-red-500 mt-4 mr-4 bg-white rounded-full rounded"
                  @click.prevent="isShowItems = false">
              ...
              <div>
                <div>
                  <span class="text-white font-bold text-sm"
                    >{{ selectedOutlet.channel }} •
                  </span>
                  <span
                    v-if="selectedOutlet.is_open"
                    class="px-4 py-1 text-sm bg-green-food rounded-lg text-white"
                    >BUKA</span
                  >
                  <span
                    v-else
                    class="px-4 py-1 bg-red-600 text-sm rounded-lg text-white"
                    >TUTUP</span
                  >
                </div>
                <div>
                  <span class="text-xl leading-loose font-bold text-white">{{
                    selectedOutlet.name
                  }}</span>
                </div>
              </div>
              ...
              <tbody v-if="listLoading">
                <tr v-for="n in 10" :key="n" class="h-12">
                  <td v-for="i in 3" :key="i">
                    <div
                      class="h-2 p-2 bg-gray-300 animate-pulse w-full rounded-lg"></div>
                  </td>
                </tr>
              </tbody>
              <tbody v-if="!listLoading">
                <tr
                  v-for="menu in items.items"
                  :key="menu.id"
                  class="hover:bg-gray-200 border-b">
                  ...
                  <td class="text-center text-text p-2">
                    {{ $toIDR(menu.selling_price) }}
                  </td>
                  <td class="text-center text-text p-2">
                    <span v-if="menu.in_stock" class="text-green-500"
                      >Aktif</span
                    >
                    <span v-if="!menu.in_stock" class="text-red-500"
                      >Tidak Aktif</span
                    >
                  </td>
                </tr>
            ...
            <div class="mt-4 flex items-center float-right">
              ...
              <div
                v-if="itemPage != 1"
                class="cursor-pointer float-right mr-2 p-3 rounded-md border-2"
                @click.prevent="changeItemPage(-1)">
                <svg
                  class=""
                  width="7"
                  height="12"
              ...
              <!-- right -->
              <div
                v-if="items.total_page > 1 && page < items.total_page"
                class="cursor-pointer float-right p-3 rounded-md border-2"
                @click.prevent="changeItemPage(1)">
                <svg
                  width="7"
              ...
              <div
                v-if="page == items.total_page || items.total_page <= 1"
                class="cursor-not-allowed float-right p-3 rounded-md border-2">
                <svg
                  width="7"
                  height="12"
      ...
      <div
        v-if="isShowItems"
        class="fixed top-0 bg-black opacity-20 left-0 z-20 w-screen h-screen"></div>
      <!-- end modal items -->
      ...
      <div
        ...
        <div class="w-1/2 bg-white rounded-fds z-40 mx-auto">
          <div class="p-4">
            <div class="bg p-6 relative rounded-fd flex gap-10 items-center">
              <div class="absolute top-0 right-0">
                <button
                  class="text-white text-sm px-3 py-1 text-red-500 mt-4 mr-4 bg-white rounded-full rounded"
                  @click.prevent="isShowSeamless = false">
                  <i class="fas fa-times-circle"></i> Close
                </button>
              </div>
              <div>
          ...
          <div class="p-4">
            <div class="md:flex md:items-center mb-6">
              <div class="md:w-1/3">
                <label
                  ...
                  for="gobiz_phone_number">
                  Nomor HP GoBiz
                </label>
              </div>
              <div class="md:w-2/3">
                <input
                  id="gobiz_phone_number"
                  v-model="connectData.phoneNumber"
                  class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-purple-500"
                  type="text"
                  placeholder="6282133110099"
                  @keydown="
            ...
            <div
              v-show="showSendOtpButton"
              class="md:flex md:items-center mb-6">
              <div class="md:w-1/3"></div>
              <div class="md:w-2/3">
                <b
                  >Dengan menekan tombol Kirim OTP, anda menyetujui bahwa
                  BotFood tidak menyimpan data OTP serta data lain yang bersifat
                  privasi ataupun rahasia</b
                >
              </div>
            </div>
            <div
            ...
            <div v-show="showTokenOtp" class="md:flex md:items-center mb-6">
              ...
              <div class="md:w-2/3">
                <b
                  >Dengan menekan tombol Submit OTP, anda menyetujui bahwa
                  BotFood tidak menyimpan data OTP serta data lain yang bersifat
                  privasi ataupun rahasia</b
                >
              </div>
            ...
            <div v-show="showTokenOtp" class="md:flex md:items-center mb-6">
              ...
              <div class="md:w-2/3">
                <button
                  class="shadow bg-green-500 hover:bg-green-400 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded"
                  type="button"
                  @click="seamlessConnect">
                  Submit OTP
                </button>
              </div>
            </div>
            <div
              v-show="errorMessageSeamless.length > 0"
              class="md:flex md:items-center mb-6">
              <div class="md:w-1/3"></div>
              <div class="md:w-2/3">
                <div
            ...
            <div
              v-show="successMessageSeamless.length > 0"
              class="md:flex md:items-center mb-6">
              <div class="md:w-1/3"></div>
              <div class="md:w-2/3">
                <div
          ...
          <div class="p-4">
            <div class="bg p-6 relative rounded-fd flex gap-10 items-center">
              <div class="absolute top-0 right-0">
                <button
                  class="text-white text-sm px-3 py-1 text-red-500 mt-4 mr-4 bg-white rounded-full rounded"
                  @click.prevent="isShowOpenStatus = false">
                  <i class="fas fa-times-circle"></i> Close
                </button>
              </div>
              <div>
              ...
              <div class="md:flex md:items-center mb-6">
                <div class="md:w-1/1">
                  <p
                    ...
                    v-html="openStatusData.text"></p>
                </div>
              </div>
              <div class="md:flex md:items-center mb-6">
                <div class="md:w-1/1 mx-auto">
                  <button
                    v-if="openStatusData.confirm"
                  ...
                  <button
                    v-if="!openStatusData.confirm"
                    class="shadow bg-gray-100 focus:shadow-outline focus:outline-none text-black font-bold py-2 px-4 rounded ml-6"
                    type="button"
                    @click="isShowOpenStatus = false">
                    Kembali
                  </button>
                </div>
              </div>
...
export default {
  ...
  methods: {
    getData(refresh = false) {
      ...
      this.listLoading = true
      const name_param = 'name=' + this.search
      const status_param =
        this.isOutletOpen == null ? '' : 'is_open=' + this.isOutletOpen
      const channel_param =
        this.outletChannel == null ? '' : 'channel=' + this.outletChannel
      this.$axios
        .get(
    ...
    handleOpenStatus(branchChannel, confirm) {
      if (confirm) {
        ...
        this.$axios
          .post(`me/seamless/open_status/${branchChannel.id}`, {
          ...
          .catch((error) => {
            this.isShowOpenStatus = false
            this.$toast.error(
              'Gagal. Hubungi kami untuk informasi lebih lanjut.',
              { duration: 2500 }
            )
          })
      } else {
    ...
    sendOTP() {
      ...
      this.$axios
        .post(`me/seamless/connect/check/${this.selectedOutlet.id}`, {
    ...
    seamlessConnect() {
      this.showTokenOtp = false
      this.$axios
        .post(`me/seamless/connect/${this.selectedOutlet.id}`, {
          phone_number: this.connectData.phoneNumber,
        ...
        .then((r) => {
          ...
          for (const key in this.data.branch_channels) {
