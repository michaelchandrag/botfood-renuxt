  ...
  <div class="flex">
    ...
    <div class="w-7/12 bg-gray-200 pl-6 pt-10">
      <div
        ...
        @click.prevent="$router.push('/outlet')">
        <img
          class="bg-white p-2 rounded-full"
          src="~/assets/svg/back.svg"
          alt="" />
        <span class="text-title"> Kembali </span>
      </div>
      <div class="h-8"></div>
      ...
      <div class="flex flex-col">
        <div class="mr-6">
          <div
            v-if="isLoading"
            class="rounded-fd w-full h-48 mr-6 bg-gray-300 animate-pulse"></div>
          <div
            v-if="!isLoading"
            class="bg p-8 rounded-fd flex gap-10 items-center">
            <div>
              <img
                v-if="data.channel == 'GrabFood'"
                class="h-32"
                src="~/assets/svg/grabfood.svg"
                alt="" />
              <img
                v-if="data.channel == 'GoFood'"
                class="h-32"
            ...
            <div>
              <div>
                ...
                <span
                  v-if="data.is_open"
                  class="px-4 py-1 bg-green-food rounded-lg text-white"
                  >BUKA</span
                >
                <span
                  v-if="!data.is_open"
                  class="px-4 py-1 bg-red-600 rounded-lg text-white"
                  >TUTUP</span
                >
              </div>
              <div>
                <span class="text-4xl leading-loose font-bold text-white">{{
                  data.name
                }}</span>
              </div>
            </div>
        ...
        <div class="mr-6">
          <div
            v-if="isLoading"
            class="rounded-fd w-full h-64 mr-6 bg-gray-300 animate-pulse"></div>
        </div>
        ...
        <div v-if="!isLoading" class="text-text bg-white rounded-fd p-8 mr-6">
          ...
          <div class="flex gap-4">
            <div class="w-3/12">
              <div
                ...
                @click.prevent="changeTab('outlet')">
            ...
            <div class="w-3/12">
              <div
                :class="{
                  'border-green-500 text-green-700 bg-green-200': tab == 'item',
                }"
                class="flex py-3 px-4 cursor-pointer text-gray-500 text-center border-2 border-gray-200 bg-gray-200 rounded-lg w-full focus:outline-none"
            ...
            <div
              v-if="tab == 'outlet'"
              class="w-6/12 cursor-pointer items-center relative">
              <div
            ...
            <div
              v-if="tab == 'item'"
              class="w-3/12 cursor-pointer items-center relative">
              <div
                class="border flex py-3 px-4 border-gray-300 rounded-lg w-full focus:outline-none"
                @click.prevent="
                  channelDropdown
            ...
            <div
              ...
              <div
                class="border flex py-3 px-4 border-gray-300 rounded-lg w-full focus:outline-none"
                @click.prevent="
                  itemDropdown ? (itemDropdown = false) : (itemDropdown = true)
                ">
                <div class="flex-auto">
              ...
              <div
                ...
                <ul class="w-full h-64 overflow-auto border-gray-300 h-48">
                  <li
                    v-for="(l, i) in listItems"
                    :key="i"
                    class="bg-white px-4 py-3 w-full border-b hover:bg-gray-200"
                    @click.prevent="getItemHistory(l.item_id, l.item_name)">
                    {{ l.item_name }}
                  </li>
                </ul>
              </div>
          ...
          <div v-if="tab == 'outlet'" class="mt-6">
              ...
              <tbody v-if="!listLoading">
                <tr
                  ...
                  <td class="text-center p-4 rounded-l-fds">
                    {{ $moment(h.created_at).format('DD MMMM YYYY') }}
                  </td>
                  <td class="text-center p-4">
                    {{ $moment(h.created_at).format('HH:mm') }}
                  </td>
                  <td class="text-center p-4">
            ...
            <div class="mt-4 flex items-center float-right">
              ...
              <div
                ...
                @click.prevent="changeOutletPage(-1)">
                <svg
                  class=""
                  width="7"
                  height="12"
              ...
              <!-- right -->
              <div
                v-if="total_page_outlet > 1 && page_outlet < total_page_outlet"
              ...
              <div
                v-if="
                  page_outlet == total_page_outlet || total_page_outlet <= 1
                "
          ...
          <div v-if="tab == 'item'" class="mt-6">
            ...
            <table class="table-auto w-full text-text">
              ...
              <tbody v-if="!listLoading">
                <tr
                  v-for="i in items_idle.history_items"
                  :key="i.id"
                  class="hover:bg-gray-300 border-b">
                  <td class="text-center p-4 rounded-l-fds">
                    {{ $moment(i.created_at).format('DD MMMM YYYY') }}
                  </td>
                  <td class="text-center p-4">
                    {{ $moment(i.created_at).format('HH:mm') }}
                  </td>
                  <td class="text-center p-4">
                    <span v-if="i.in_stock == 1" class="text-green-500"
                      >Aktif</span
                    >
                    <span v-if="i.in_stock == 0" class="text-red-500"
                      >Tidak Aktif</span
                    >
                  </td>
                </tr>
            ...
            <div class="mt-4 flex items-center float-right">
              ...
              <div
                v-if="page_item != 1"
                class="cursor-pointer float-right mr-2 p-3 rounded-md border-2"
                @click.prevent="changePageItem(-1)">
                <svg
                  class=""
                  width="7"
                  height="12"
              ...
              <!-- right -->
              <div
                v-if="total_page_item > 1 && page_item < total_page_item"
                class="cursor-pointer float-right p-3 rounded-md border-2"
                @click.prevent="changePageItem(1)">
                <svg
                  width="7"
              ...
              <div
                v-if="page == total_page_item || total_page_item <= 1"
                class="cursor-not-allowed float-right p-3 rounded-md border-2">
                <svg
                  width="7"
                  height="12"
...
export default {
  ...
  methods: {
    changeTab(val) {
      ...
      if (val == 'item') {
        this.$axios
          .get('me/branch_channel/' + id + '/items_idle?issued_at=' + date)
          .then((r) => {
    ...
    changePageNumberItem() {
      ...
      const date = this.$moment(this.date).format('YYYY-MM-DD')
      const page = 'page=' + this.page_item
      this.$axios
        .get(
          'me/history_item/' +
            id +
