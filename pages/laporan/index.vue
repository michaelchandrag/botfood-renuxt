        ...
        <div v-if="!isLoading" class="bg-white rounded-fd p-8 mr-6 text-text">
          ...
          <div class="flex gap-4">
            ...
            <div class="w-3/12 cursor-pointer items-center relative">
              ...
              <div
                v-if="statusDropdown"
                class="absolute -ml-24 right-0 shadow-sm rounded-b-fds">
                <client-only>
                  <date-picker
                    v-model="date"
                    class="w-96"
            ...
            <table class="table-auto w-full">
              <thead class="border-b">
                <tr>
                  ...
                  <th
                    class="py-4 text-text cursor-pointer"
                    :class="sortKey === 'active_time' ? 'filter' : ''"
                    @click.prevent="
                      ;(sortKey = 'active_time'),
                        sortValue === 'desc'
                          ? (sortValue = 'asc')
                  ...
                  <th
                    class="py-4 text-text cursor-pointer"
                    :class="
                      sortKey === 'outlet_item_not_active_time' ? 'filter' : ''
                    "
                    @click.prevent="
              ...
              <tbody v-if="!listLoading">
                <tr
                  v-if="
                    !listLoading &&
                    data.branch_channel_availability_reports.length <= 0
                  ">
              ...
              <tbody v-if="!listLoading">
                <tr
                  v-for="channel in data.branch_channel_availability_reports"
                  :key="channel.id"
                  class="hover:bg-gray-200 border-b">
                  <td class="text-center text-text p-4 rounded-l-fds">
                    {{ channel.branch_channel_name }}
                  </td>
                  <td class="text-center text-text p-4">
                    {{ channel.branch_channel_channel }}
                  </td>
                  <td class="text-center text-text p-4 rounded-r-fds">
                    <span
                      v-if="
                        $moment
                          .duration(channel.active_time, 'seconds')
                          .hours() > 0
                      "
                      >{{
                        $moment.duration(channel.active_time, 'seconds').hours()
                      }}
                      jam</span
                    >
                    <span
                    ...
                    <div v-else>
                      <span
                        v-if="
                          $moment
                            .duration(
                              channel.outlet_item_not_active_time,
                              'seconds'
                            )
                            .hours() > 0
                        "
