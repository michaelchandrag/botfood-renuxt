  ...
  <div>
    ...
    <div class="bg-gray-200 wrapper-content have-right-sidebar">
      ...
      <div class="flex flex-col ">
        ...
        <!-- table start -->
        <div v-if="!isLoading" class="bg-white rounded-fd p-5 text-text">
          ...
          <div class="w-full">
            <div class="flex flex-wrap items-center -m-2">
              <div class="w-full xl:w-3/12 lg:w-full p-2">
                <div class="relative">
                  <form @submit.prevent="getData">
                    <input @change.prevent="getData" type="text"
                      ...
                      v-model="search" placeholder="Nama Outlet">
                  </form>
              ...
              <div class="w-6/12 xl:w-3/12 lg:w-6/12 cursor-pointer items-center relative p-2">
                ...
                <div v-if="channelDropdown" class="absolute w-full shadow-sm rounded-b-fds" style="z-index:2;">
          ...
          <div class="mt-6">
            <div class="table-responsive">
              <table class="table-auto w-full">
                <thead class="border-b">
                  <tr>
                    <th class="py-4 text-text cursor-pointer" :class="sortKey==='name'?'filter':''"
                      @click.prevent="sortKey='name', sortValue==='desc' ? sortValue='asc': sortValue='desc',getData()">
                      Nama Outlet
                    ...
                    <th class="py-4 text-text cursor-pointer" :class="sortKey==='channel'?'filter':''"
                      @click.prevent="sortKey='channel', sortValue==='desc' ? sortValue='asc': sortValue='desc',getData()">
                      Channel
                    ...
                    <th class="py-4 text-text cursor-pointer"
                      :class="sortKey==='outlet_item_not_active_time'?'filter':''"
                      @click.prevent="sortKey='outlet_item_not_active_time', sortValue==='desc' ? sortValue='asc': sortValue='desc',getData()">
                      Menit Item Tidak Aktif (Rata-rata)
                ...
                <tbody v-if="!listLoading">
                  <tr v-for="channel in data.branch_channel_availability_reports" :key="channel.id"
            ...
            <div class="mt-4 flex items-center float-right">
              <div class="float-right p-2">
                <form>
                  <input @change="changePageNumber()"
                    class="w-20 h-10 text-center border-2 rounded-md focus:outline-none" type="text" inputmode="numeric"
                    pattern="[0-9]*" v-model="page">
                  of {{data.total_page}}
                </form>
              </div>
  ...
  export default {
    ...
    methods: {
      getData() {
        this.listLoading = true
        this.$axios.get(
            `me/report/atp?date=${this.date}&sort_key=${this.sortKey}&sort_value=${this.sortValue}&branch_channel_name=${this.search}&branch_channel_channel=${this.outletChannel}`
          )
          .then(r => {
