<template>
  <div>
    ...
    <div class="bg-gray-200 wrapper-content">
      ...
      <div>
        <span class="text-title">Laporan Variant</span>
      </div>
      <div>
        <span class="items-center text-orange-500 inline-block align-middle">
          ...
          <span class="ml-2 text-orange-500">Baru tersedia untuk GoFood</span>
        </span>
      </div>
      ...
      <div v-if="!isLoading" class="bg-white rounded-fd p-8 text-text">
        <div class="flex flex-wrap -m-2">
          ...
          >
            ...
            >
              <div class="flex-auto">
                <span v-if="itemStatus == true">Tersedia</span>
                <span v-if="itemStatus == false">Tidak Tersedia</span>
                <span v-if="itemStatus == null">Semua Status (Tersedia/Tidak Tersedia)</span>
              </div>
              <div>
                <svg
            ...
            >
              <ul class="w-full border-gray-300">
                ...
                >
                  Semua Status (Tersedia/Tidak Tersedia)
                </li>
              </ul>
            </div>
          </div>
          ...
          <table class="table-auto w-full">
            <thead>
              <tr class="border-b">
                ...
                <th
                  class="py-4 text-text text-center cursor-pointer"
                  :class="sortKey === 'variant_category_name' ? 'filter' : ''"
                  @click.prevent="
                    (sortKey = 'variant_category_name'),
                      sortValue === 'desc'
                        ? (sortValue = 'asc')
                        : (sortValue = 'desc'),
                      getData(true)
            ...
            <tbody v-if="!listLoading">
              ...
              >
                ...
                <td
                  ...
                  style="max-width: 200px"
                >
                  {{ item.variant_category_name }}
                </td>
                <td class="text-center text-text p-4">
                  <span v-if="item.in_stock" class="text-green-500">Tersedia</span>
...
export default {
  ...
  methods: {
    download() {
      ...
      this.$axios({
        method: 'GET',
        url: `me/variant/export?${item_name}&${stock}&${channel}`,
        responseType: 'blob',
      }).then(r => {
        ...
        fileLink.href = url;
        fileLink.setAttribute('download', 'variant.xlsx');
    ...
    getData(refresh = false) {
      ...
      this.$axios
        .get(
          `me/variant/list?${item_name}&${outletName}&${stock}&${channel}&sort_key=${this.sortKey}&sort_value=${this.sortValue}&${page}`
        )
        .then((r) => {
