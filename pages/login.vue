<template>
  <div class="h-screen w-full bg-login bg-cover flex items-center p-5">
    <div class="bg-white max-w-md rounded-fd mx-auto shadow-xl p-5 w-full p-5">
      <div class="mx-auto h-full flex">
        <form class="mx-auto w-full" @submit.prevent="userLogin">
          <div class="text-center">
            <img class="mx-auto" style="height:25px;margin-top:30px;" src="~/assets/svg/logo.svg" alt="">
          </div>
          <div style="margin-top:40px">
            Brand <br />
            <input v-model="login.slug" style="height:56px;"
              :class="{'bg-red-200': isError, 'border' : isError, 'border-red-400' : isError}"
              class="w-full px-5 py-1 text-sm text-gray-700 bg-gray-200 rounded-fds focus:border focus:border-gray-400 focus:outline-none focus:bg-gray-100"
              type="text" placeholder="Masukkan nama brand" required>
          </div>
          <div style="margin-top:40px">
            Password <br />
            <input v-model="login.password" style="height:56px;"
              :class="{'bg-red-200': isError, 'border' : isError, 'border-red-400' : isError}"
              class="w-full px-5 py-1 text-sm text-gray-700 bg-gray-200 rounded-fds focus:border focus:border-gray-400 focus:outline-none focus:bg-gray-100 text-sm"
